...
Bonus Lab:
You setup a RESTful server on your AWS instance (take a look at "Quick Restful Node Server Setup" under [RESTful and JWT](https://github.com/NikkiNgNguyen/ransomware_cecs378/blob/master/RESTfulandJWT.md)). You want to make sure that your server has a simple DB with at least two API's. One is to POST a public/private key pair (stored in your MongoDB). The other is a GET request which contains a Public key in the hearder and it returns the corresponding private key in response.
